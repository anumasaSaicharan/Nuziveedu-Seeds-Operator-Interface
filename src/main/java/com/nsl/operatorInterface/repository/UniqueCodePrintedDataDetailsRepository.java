...
public interface UniqueCodePrintedDataDetailsRepository extends JpaRepository<UniqueCodePrintedDataDetails, Long> {
	...
	@Query(value = "SELECT TOP(:limit) * FROM UNIQUE_CODE_PRINTED_DATA_DETAILS WHERE ACTIVE = 1 AND USED = 0 AND CODES_YEAR = :year ORDER BY SERIAL_NUMBER ASC", nativeQuery = true)
