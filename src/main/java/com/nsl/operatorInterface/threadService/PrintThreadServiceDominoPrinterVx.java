...
//	private static int whileLoopFlag = 0;
	...
	public PrintThreadServiceDominoPrinterVx() {
	...
	public static void setStopThreadVariable(boolean stopThreadVariable) {
		PrintThreadServiceDominoPrinterVx.stopThreadVariable = stopThreadVariable;
	}
	...
	public static void setWhileLoopFlag(int whileLoopFlag) {
		PrintThreadServiceDominoPrinterVx.whileLoopFlag = whileLoopFlag;
	}
		...
		try {
					synchronized (session) {
						...
						/*	if("g".equalsIgnoreCase(printOperatorInterfaceDetails.getPackUnit())) {
								unitPrice="INR: "+String.format("%.2f",printOperatorInterfaceDetails.getUnitPrice().doubleValue())+" Per gm";
							}
							if("kg".equalsIgnoreCase(printOperatorInterfaceDetails.getPackUnit())) {
								unitPrice="INR: "+String.format("%.2f",printOperatorInterfaceDetails.getUnitPrice().doubleValue())+" Per kg";
							}*/
							
							mrp= String.format("%.2f",printOperatorInterfaceDetails.getMrp().doubleValue());
							...
							if("g".equalsIgnoreCase(printOperatorInterfaceDetails.getPackUnit())) {
								...
								unitPrice=String.format("%.2f",printOperatorInterfaceDetails.getUnitPrice().doubleValue())+"";
							}
							...
							else if("LT".equalsIgnoreCase(printOperatorInterfaceDetails.getPackUnit()) || "litre".equalsIgnoreCase(printOperatorInterfaceDetails.getPackUnit())) {
								unitPrice=String.format("%.2f",printOperatorInterfaceDetails.getUnitPrice().doubleValue())+"";
							}
							}
							...
							String tempSelCmd="LOADPROJECT "+templateName+".lbl\r\n";
							log.info("TEMP_SEL_CMD=DOMINO_VX=>"+tempSelCmd);     
							boolean flag=false;
							boolean selCmdFlag=true;
							boolean slaCmdFlag=true;
							while(selCmdFlag)
							{
								String resp = sendCommandToPrinter(tempSelCmd,socket);
								log.info("TEMP_SEL_CMD_RESP==>"+resp);//AnswerPrintDesign=001.Design,1,0,0,0,PromptInfo,0, OR error								
									selCmdFlag=false;								
							}
							...
							int sentCount=0;
							int bufferSize=Integer.parseInt(appConfig.getProperty("BUFFER_SIZE"));//DEFINE on 28-03-2023
							if (noOfPrints > 20)
								sentCount = bufferSize;
							else
								sentCount = noOfPrints;
							
							int totalSentCount=0;
							int queSize= getBufferStatus(socket);
							//log.info("==PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==queSize==>"+queSize);
							boolean batchFlag=false;
							
							String setPrinterReadyCmd="MARK START\r\n";
							String setPrinterReadyCmdResp = sendCommandToPrinter(setPrinterReadyCmd,socket);
							log.info("setPrinterReadyCmdResp ==>"+setPrinterReadyCmdResp);
							if("OK".equalsIgnoreCase(setPrinterReadyCmdResp))
								flag=true;
							
							if(queSize<=bufferSize) 
								batchFlag=true;
							while(batchFlag)
							{
								synchronized (this) {
									log.info("=1A=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==SYNCCHRO=LOOP_SIZE==>"+printedDataDetailsList.size());
									log.info("=1B===bufferSize===>"+bufferSize);
									log.info("=1C===queSize===>"+queSize);
									List<UniqueCodePrintedDataDetails> sendBufferList=new ArrayList<>();
									sentCount=bufferSize-queSize;////need to define
									log.info("=2=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==sentCount=="+sentCount);
									if(sentCount>0) 
									{
										...
										*/
										...
										log.info("=3=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==sendBufferList.size==>"+sendBufferList.size());
									}
									if(sendBufferList.size()>0)
									{
										...
										for(UniqueCodePrintedDataDetails qrUid:sendBufferList)
										{
											...
											if(cnt==0)
											{
												log.info("==COUNT==>"+cnt+"==>==SerialNo==>"+qrUid.getSerialNumber());
												log.info("==PRINT_JOB_ID=StopThreadVariable()=>"+PrintThreadServiceDominoPrinterVx.isStopThreadVariable());
												
												if(!PrintThreadServiceDominoPrinterVx.isStopThreadVariable() && flag)
												{
													log.info(":: ENTERTO SAVE ::");
...
//													qrUid.setGtinNumber(printOperatorInterfaceDetails.getGtinNumber());
													...
													if(printOperatorInterfaceDetails.getUseShortUrl().equalsIgnoreCase("YES")) {
														...
														String bufferDataCmd = "BUFFERDATA "+count+" \""+batchNo+"\" \""+mfgDt1+"\" \""+expDt1+"\" \""+mrp+"\" \""+unitPrice+"\" \""+url+"\"\r\n";
														 bufferDataCmdResp = sendCommandToPrinter(bufferDataCmd,socket);
													}
													...
													log.info("==PRINT_JOB_ID==>"+printOperatorInterfaceDetails.getId()+"==bufferDataCmdResp==>"+bufferDataCmdResp);
													printedCodesHistory.setPrintedUrl(url);
													printedCodesHistoryRepository.save(printedCodesHistory);
													//Thread.sleep(300);
													log.info("==PRINT_JOB_ID==>" + printOperatorInterfaceDetails.getId()
															+ "==JDI_LOOP_COUNT==>" + count);
												} else {
													batchFlag = false;
										...
										log.info("=5=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==LOOP_END");
										...
										log.info("=6=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==alreadyPrintedQty==>"+alreadyPrintedQty);
									...
									log.info("=7==PRINT_JOB_ID==>=PrintThreadServiceWorking.getWhileLoopFlag()==>"+printOperatorInterfaceDetails.getId()+"=="+PrintThreadServiceDominoPrinterVx.getWhileLoopFlag());
									totalSentCount=totalSentCount+sentCount;
									log.info("=8=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==printedDataDetailsList=SIZE="+printedDataDetailsList.size());
									log.info("=9=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==sendBufferList=SIZE="+sendBufferList.size());
								    ...
								    log.info("=10=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==unmatchedList==>"+unmatchedList.size());
								    ...
								    log.info("=11=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==printedDataDetailsList==>"+printedDataDetailsList.size());
								    
									int qsz = getBufferStatus(socket);
									log.info("=12=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==qszCmdResp==>"+qsz);
									//if(qsz==0 || qsz==20)
										queSize= qsz;
									
									log.info("=13A=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==queSize==>"+queSize);
									log.info("=13B=totalSentCount=="+totalSentCount);
									log.info("=13C=noOfPrints=="+noOfPrints);
									...
									log.info("=14=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==sentCount==>"+sentCount);
									log.info("=15=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==totalSentCount==>"+totalSentCount);
								}//Synchronized end
								
								//break while loop
								log.info("=16==PRINT_JOB_ID==>=PrintThreadServiceWorking.getWhileLoopFlag()==>"+printOperatorInterfaceDetails.getId()+"=="+PrintThreadServiceDominoPrinterVx.getWhileLoopFlag());
								if(PrintThreadServiceDominoPrinterVx.getWhileLoopFlag()==1) {
									break;
								}
								
							}//While end
							log.info("=17=PRINT_JOB_ID=="+printOperatorInterfaceDetails.getId()+"==totalSentCount==>"+totalSentCount+"==FOR LOOP count=>"+count);
							
							if (PrintThreadServiceDominoPrinterVx.isStopThreadVariable()) {
			...
			public Object findObject(Session session, String queryString, Object... values) throws DataAccessException {
			...
			public static int getBufferStatus(Socket socket) {
				int size = 0;
				if (socket != null) {
					if (socket.isConnected()) {
						OutputStream output = null;
						boolean flag = true;
						String sndCmd = "GETBUFFERSTATUS\r\n";
						while (flag) {
							try {
								BufferedReader reader = null;
								output = socket.getOutputStream();
								...
								log.info("QSZ_CMD_RESP==>" + response);
								if (response.contains("RESULT GETBUFFERSTATUS")) {
									log.info("QSZ_Q_SIZE=>" + response.split(" ")[2]);
									size = Integer.parseInt(response.split(" ")[2]);
									// log.info("Q_size==>"+size);
									flag = false;
								} else {
									flag = true;
								}
						...
						log.info("RETURN_QUEE_SIZE==>" + size);
					}
				}
				return size;
			}
			...
			@SuppressWarnings("unused")
			public String createTemplateWithQRCodeAndValues(String pdfFileName,String qrcodeImgFilePath,String batchNo,String mfgDt,String expDt,String mrp)
			{
